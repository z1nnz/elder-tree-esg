...
先讀 `../../../docs/design/skill-library.md`，先找出真正的父層限制，
不要對每個溢出一律加上 `Expanded` 或固定高度；保留系統字級放大。
若沒有 Dart／Flutter MCP，使用已安裝的 Flutter 除錯、分析與測試工具；
不要因此自動安裝伺服器、升級 SDK 或更換來源中繼資料提到的模型。
