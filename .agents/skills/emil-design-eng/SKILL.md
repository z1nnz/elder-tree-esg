...
先完整閱讀 `../../../docs/design/skill-library.md`。此技能用來精修可見的
質感：按壓回饋、轉場節奏、層次與元件細節；直接處理使用者的畫面需求，
不以課程廣告或制式自我介紹取代工作。來源與修改見 `NOTICE.md`。

- 下方動畫、陰影與曲線建議是設計參考，不是每個畫面都要套用的規範。
  沿用現有元件；不因這份指南自動新增 Motion、GSAP 或其他套件。
- 長者閱讀及常用操作優先保持清楚、快速；不讓文字模糊、等待入場、縮放
  或長按成為完成操作的必要條件。重要結果保留文字，不只靠動畫傳達。
- 尊重減少動態效果設定；可完全停用非必要動畫。降低動態效果時也要保留
  操作與狀態，不能只是把原來移動改成另一種持續閃爍。
- 實際效能取決於瀏覽器、動畫屬性、合成條件及套件版本；原文的 GPU／
  主執行緒說法不是保證，需在目標裝置量測，不能據此自動改寫實作。
- Flutter 使用對應的原生動畫與語意元件，不直接移植 CSS／React 程式。
  複製視覺層作動畫時，避免重複焦點、朗讀與互動；手勢要有可點替代方式。
- 回報時用「修改前／修改後／理由」說清楚可見差異，並區分靜態截圖、
  錄影與實機操作的驗證範圍；不把作者偏好當作使用者研究結論。
