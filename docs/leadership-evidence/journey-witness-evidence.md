...
合併請求：[GitHub #48](https://github.com/z1nnz/elder-tree-esg/pull/48)
...
- 隱私清理：定位事件回執不保存健康步數；常駐 API 每分鐘清理四小時前仍未結束工作階段的最新精確座標、暫存步數與來源。定位事件與所有結束／逾時清理共用工作階段交易鎖，避免並行事件在清除後寫回精確資料。
