...
### 2026-09-12：單島塑形與桌面展示（開發中）

- 延續主要負責人 `@z1nnz` 的外觀優先決策，重塑樹冠主次、島岸台地與偏移岩體；林地改依實際溪流網格保留河道空間。沒有增加面數或新增人物、小鎮玩法。
- [實景、建置雜湊與驗收紀錄](世界樹島形與桌面展示-2026-09-12.md)：Unity 33 項、Blender 5 項通過，桌面展示程式已成功建置並實際載入新模型，縮放與回復視角已觀察。
- 自動化拖曳旋轉未取得明確通過結果，手機及最終美術仍未驗收；不代填本人親自操作。更正前一輪溪流接縫的未證實成因，保留修正依據。版本續於草稿 [#54](https://github.com/z1nnz/elder-tree-esg/pull/54)，不推定已合併 `main`。
- 拖曳追查：暫時診斷取得同幀、零位移的原始輸入事件；排除當次介面攔截，但未證明有效手動拖曳是否失效。已移除診斷元件、重新建置乾淨展示版，並在上述文件保存事件與新建置識別；手動驗收仍待回覆，不記作修復完成。
